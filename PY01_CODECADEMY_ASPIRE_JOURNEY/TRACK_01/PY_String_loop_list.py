...
#------------------------------------------------
daily_sales_replaced_splitted = daily_sales.replace(';,;',':').split(",")

cleaned_sales = []

for transaction in daily_sales_replaced_splitted:
    transaction_list = transaction.split(":")

    customer = transaction_list[0].strip()
    amount = transaction_list[1].strip().replace('$','')
    amount = float(amount)
    colors = transaction_list[2].strip().split('&')
    date_sold = transaction_list[3].strip()

    transaction_tuple = (customer, amount, colors, date_sold)

    cleaned_sales.append(transaction_tuple)
...
color_list = []
for record in cleaned_sales:
    color_list.append(record[2])

def count_thread_type(color_list, chosen_color):

    chosen_color_count = 0
    for thread in color_list:
        for color in thread:
            if color == chosen_color:
                chosen_color_count += 1
    return (chosen_color,chosen_color_count)
